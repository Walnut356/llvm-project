...
bool RustUserExpression::Parse(
  ...
  if (!m_expr) {
    diagnostic_manager.PutString(lldb::eSeverityError, status.AsCString());
    return false;
  }
...
lldb::ExpressionResults RustUserExpression::DoExecute(
    DiagnosticManager& diagnostic_manager,
    ExecutionContext& exe_ctx,
  ...
  if (!value) {
    diagnostic_manager.PutString(lldb::eSeverityError, error.AsCString());
